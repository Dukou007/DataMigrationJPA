 ...
 *
 */
public class QualityPageDataWorker extends QualityBaseDataWorker implements Runnable {

	protected static final int _DEFAULT_PAGE_SIZE = 1000;// 默认分页大小
	protected Integer pageSize = _DEFAULT_PAGE_SIZE;
	int pageIndex = 1;

	public QualityPageDataWorker(BlockingQueue<QualityBaseData> queueSource, QualityQueryModel querySource, Integer pageSize)
	        throws Exception {
	...
	QualityTestQuery sourceQuery;

	public QualityPageDataWorker(BlockingQueue<QualityBaseData> queue, QualityQueryModel testQuery, Integer pageSize,
	        QualityTestQuery sourceQuery) throws Exception {
		...
		try {
			...
			while (true) {
				...
				if (map.size() == 0) {
					...
					QualityPageData data = new QualityPageData();
					data.setMap(map);
			...
			if (map.size() > 0) {
				// 最后一组map
				QualityPageData page = new QualityPageData();
				page.setMap(map);
