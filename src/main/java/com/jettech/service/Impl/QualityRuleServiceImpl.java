...
public class QualityRuleServiceImpl implements IQualityRuleService {
	...
	public ResultVO delete(int id) throws Exception {
		...
		for (QualitySuite qualitySuite : qualitySuites) {
			...
			List<QualityTestQuery> qualityTestQuerys =  qualityTestQueryRepository.findByQualitySuiteId(qualitySuiteId);
			if(qualityTestQuerys.size()!=0) {
				return new ResultVO(false, StatusCode.ERROR, "删除失败");
//				throw new Exception("该质量规则正在被quality_test_query使用");			
			}
	...
	public ResultVO getAll(int pageNum, int pageSize) {
		...
		Page<QualityRule> pageQualityRule = qualityRuleRepository
				.findAll(pageRequest);
