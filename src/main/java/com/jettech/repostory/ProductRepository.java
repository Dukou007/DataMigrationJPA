...
public interface ProductRepository extends JpaRepository<Product, Integer>, JpaSpecificationExecutor<Product> {
	...
	@Query(value = "SELECT  B.parent_id,GROUP_CONCAT(B.id) id,GROUP_CONCAT(B.name) `name` FROM " + "("
			...
			+ "		SELECT P.id,P.`name`,P.parent_id FROM product P WHERE parent_id IN(SELECT ID FROM product)" + "	) A"
			+ "	INNER JOIN (SELECT C.id,C.`name` FROM  product C WHERE C.parent_id IS NULL) D" + "	ON A.parent_id=D.id"
			+ ") B" + "GROUP BY parent_id", nativeQuery = true)
