...
public class DataFieldController {
	...
	public ResultVO copyDataField(@PathParam(value = "id") Integer id,
			@PathParam(value = "name") String name) {
		try {
			return testFieldService.copyDataField(id, name);
		} catch (Exception e) {
			e.printStackTrace();
			return new ResultVO(false, StatusCode.ERROR, "复制失敗");
		}
	}
	...
	public ResultVO getFieldsByTableID(
			@PathParam(value = "tableID") Integer tableID,
			@PathParam(value = "fieldName") String fieldName) {
		Map<String, Object> resultmap = new HashMap<String, Object>();
		List<TestFieldVO> arrvolist = new ArrayList<>();
		try {
			List<DataField> dtlist = testFieldService.findFieldNameByTableID(
					tableID, fieldName);
